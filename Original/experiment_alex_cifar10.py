...
with open("_models/alexnet_cifar10_gpu_results(train).pkl", "w") as f:
...
# Specify thresholds
# (INFERENCE) thresholds = [0.0001, 0.001, 0.005, 0.01, 0.05, 0.1, 0.5, 0.75, 1., 5., 10.]


#GPU
# (INFERENCE) branchyNet.to_cpu()
# (INFERENCE) g_ts, g_accs, g_diffs, g_exits = utils.screen_branchy(branchyNet, x_test, y_test, thresholds, batchsize=TEST_BATCHSIZE, verbose=True)
...
# (INFERENCE) utils.branchy_table_results('AlexNet', g_baseacc, g_basediff, g_accs, g_diffs, g_exits, g_ts)

# Save model/data

# (INFERENCE) with open("_models/alexnet_cifar10_gpu_results.pkl", "w") as f:
